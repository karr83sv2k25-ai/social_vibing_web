...
export default function GroupAudioCallScreen() {
  ...
  const audioQueueRef = useRef({}); // Queue for each user's audio chunks
  const isPlayingRef = useRef({}); // Track if audio is currently playing for each user
      ...
      try {
        ...
        if (auth.currentUser) {
          ...
          if (userSnap.exists()) {
            ...
            else if (userData.firstName) userName = userData.firstName;
            else if (auth.currentUser.displayName) userName = auth.currentUser.displayName;
            else if (auth.currentUser.email) userName = auth.currentUser.email.split('@')[0];
            
            console.log('[User] Fetched user name:', userName);

            setCurrentUser({
              id: userId,
              name: userName,
              profileImage: userData.profileImage || userData.avatar || userData.photoURL || null,
  ...
  useEffect(() => {
    ...
    const joinRoom = async () => {
      try {
        ...
        // Check if room exists and if user is already in it
        ...
        if (roomSnap.exists()) {
          ...
          if (!userExists) {
            // Add current user to participants (avoid duplicates)
            ...
            console.log('[Call] User already in call, skipping duplicate join');
          }
        } else {
    ...
    return () => {
      ...
      if (recordingRef.current || recording) {
        console.log('[Call] Stopping recording on unmount');
        stopRecording().catch(() => {});
      }
        ...
        const leaveRoom = async () => {
          try {
            ...
            if (roomSnap.exists()) {
              ...
              console.log('[Call] Leaving room, remaining participants:', updatedParticipants.length);

              if (updatedParticipants.length === 0) {
                // Delete room if no participants left
                console.log('[Call] Last user leaving, deleting room');
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snap) => {
        if (snap.exists()) {
          ...
          // Check if current user is alone (only 1 participant left)
          if (participantsList.length === 1 && participantsList[0].userId === currentUser?.id) {
            console.log('[Call] Only user left in call, ending call...');
            setIsCallActive(false);
            
            // Clean up room since no one else is there
            try {
              await deleteDoc(roomRef);
              console.log('[Call] Empty room deleted');
            } catch (e) {
              console.log('[Call] Error deleting empty room:', e);
            }
            
            Alert.alert('Call Ended', 'You are the last person in the call');
            navigation.goBack();
            return;
          }

          // Check if room is completely empty (shouldn't happen, but defensive)
          if (participantsList.length === 0) {
            console.log('[Call] Room is empty, ending call...');
            setIsCallActive(false);
          ...
          console.log('[Call] Room no longer exists, ending call...');
          setIsCallActive(false);
          Alert.alert('Call Ended', 'The audio call has ended');
          navigation.goBack();
        }
          ...
          if (data.isSpeaking && data.audioUrl) {
            ...
            if (chunkTimestamp) {
              try {
                ...
                isRecentChunk = timeDiff < 6000; // 6s window - better buffering for smooth playback
  ...
  // Play audio chunk with queue system for continuous playback
  const playAudioChunk = async (userId, audioUrl) => {
    ...
    if (!audioQueueRef.current[userId]) {
      audioQueueRef.current[userId] = [];
      isPlayingRef.current[userId] = false;
    }

    // Add to queue
    audioQueueRef.current[userId].push(audioUrl);
    console.log(`[Audio] Added to queue for ${userId}: ${audioUrl.substring(0, 60)}... (Queue size: ${audioQueueRef.current[userId].length})`);

    // Start playing queue if not already playing
    if (!isPlayingRef.current[userId]) {
      playNextInQueue(userId);
    }
  };

  // Play next audio chunk in queue
  const playNextInQueue = async (userId) => {
    // Check if queue has items
    if (!audioQueueRef.current[userId] || audioQueueRef.current[userId].length === 0) {
      isPlayingRef.current[userId] = false;
      console.log(`[Audio] Queue empty for ${userId}`);
      return;
    }

    isPlayingRef.current[userId] = true;
    const audioUrl = audioQueueRef.current[userId].shift(); // Get next chunk
    console.log(`[Audio] Playing from queue for ${userId}: ${audioUrl.substring(0, 60)}... (Remaining: ${audioQueueRef.current[userId].length})`);

    try {
      // Configure audio mode for optimal streaming playback
      ...
      const { sound } = await Audio.Sound.createAsync(
        { uri: audioUrl },
        {
          ...
          volume: 1.0,
          isMuted: false,
          rate: 1.0, // Normal speed for natural sound
      ...
      console.log(`[Audio] ✓ Sound playing for ${userId}`);
      ...
      activeSoundsRef.current[userId] = sound;

      // Cleanup and play next when finished
      sound.setOnPlaybackStatusUpdate((status) => {
        if (status.isLoaded && status.didJustFinish) {
          console.log(`[Audio] Chunk finished for ${userId}, playing next...`);
          ...
          // Play next chunk in queue immediately
          setTimeout(() => playNextInQueue(userId), 0);
      ...
      // Try to play next chunk anyway
      setTimeout(() => playNextInQueue(userId), 100);
    }
  };
  ...
  const toggleMute = async () => {
    ...
    // Always stop recording first to ensure cleanup
    await stopRecording();

    if (!newMutedState) {
      // Wait for complete cleanup before starting new recording
  ...
  const startRecording = async () => {
    try {
      ...
      }, 500); // 500ms = smoother audio with minimal latency

      audioChunkIntervalRef.current = interval;
      setAudioChunkInterval(interval);
      console.log('[Recording] ✅ Chunk upload interval started (500ms for smooth audio)');
    } catch (err) {
  ...
  const stopRecording = async () => {
    ...
    // Clear interval first to prevent new chunks
    if (audioChunkIntervalRef.current) {
      clearInterval(audioChunkIntervalRef.current);
      audioChunkIntervalRef.current = null;
  ...
  const endCall = async () => {
    Alert.alert('End Call', 'Are you sure you want to leave the call?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: async () => {
          console.log('[Call] User manually leaving call...');
          ...
          try {
            ...
            if (roomSnap.exists()) {
              ...
              console.log('[Call] Removing user, remaining:', updatedParticipants.length);

              if (updatedParticipants.length === 0) {
                // Delete room if no participants left
                console.log('[Call] Last participant, deleting room');
                await deleteDoc(roomRef);
              } else {
                await updateDoc(roomRef, {
                  participants: updatedParticipants,
            ...
            console.log('[Call] Successfully left call');
          } catch (e) {
  ...
  useEffect(() => {
    return () => {
      ...
      // Clean up audio queues
      try {
        audioQueueRef.current = {};
        isPlayingRef.current = {};
        console.log('[Cleanup] Audio queues cleared');
      } catch (e) {
        // Ignore cleanup errors
      }
